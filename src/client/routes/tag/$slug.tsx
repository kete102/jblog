...
import { Eye, Heart, BookOpen, ArrowUpDown, Hash } from 'lucide-react'
...
function TagPage() {
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <p className="text-base-content/50 text-sm">Cargando categoría…</p>
      </div>
    )
  }
  ...
  if (isError || !data) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <p className="text-error text-sm">No se pudo cargar la categoría.</p>
      </div>
    )
  }

  const posts: PostSummary[] = data.posts.map((p) => ({ ...p, status: 'published' as const }))
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-10">
      ...
      <div className="flex items-center gap-3 mb-8">
        <Hash className="w-7 h-7 text-primary shrink-0" />
        <h1
          style={{ viewTransitionName: `tag-title-${slug}` }}
